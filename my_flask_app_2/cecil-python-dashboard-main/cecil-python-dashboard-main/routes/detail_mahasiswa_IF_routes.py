...
if_bp = Blueprint('if_bp', __name__)
...
# S1 Informatika Prodi ID
ID_PRODI_INFORMATIKA = 4 # Corrected ID based on common mapping, previously 3 in provided code
...
def calculate_semester(angkatan_tahun_masuk):
    ...
    if current_month >= 8: # August-December (Odd)
        current_semester_offset = 1
    elif current_month >= 2 and current_month <= 7: # February-July (Even)
...
@if_bp.route('/dekannilaiif')
def dekan_nilai_if():
    return render_template('dekan_nilai_S1if.html')

# S1 INFORMATIKA STUDENT DETAILS (FOR DEKAN_NILAI_S1IF.HTML)
@if_bp.route('/api/mahasiswa-if', methods=['GET'])
def api_mahasiswa_if():
    current_year = datetime.now().year
    ...
    for angkatan in angkatan_list:
        try:
            ...
            for mhs in data:
                if (
                    int(mhs.get("id_program_studi", -1)) == ID_PRODI_INFORMATIKA and
...
def detail_mahasiswa(nim):
    ...
    return render_template("detail_nilai_S1if.html", mahasiswa=mahasiswa_detail)