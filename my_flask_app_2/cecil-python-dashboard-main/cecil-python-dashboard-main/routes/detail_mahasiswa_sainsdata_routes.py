...
# S1 SAINS DATA Prodi ID (based on the system)
ID_PRODI_SAINS_DATA = 58 
...
def calculate_semester(angkatan_tahun_masuk):
    ...
    elif current_month >= 2 and current_month <= 7: # February-July
...
# S1 SAINS DATA STUDENT DETAILS (FOR DEKAN_NILAI_SD.HTML)
...
def api_mahasiswa_sd():
    ...
    for angkatan in angkatan_list:
        try:
            ...
            for mhs in data:
                if (
                    int(mhs.get("id_program_studi", -1)) == ID_PRODI_SAINS_DATA and
...
def detail_mahasiswa(nim):
    ...
    return render_template("detail_nilai_sd.html", mahasiswa=mahasiswa_detail)