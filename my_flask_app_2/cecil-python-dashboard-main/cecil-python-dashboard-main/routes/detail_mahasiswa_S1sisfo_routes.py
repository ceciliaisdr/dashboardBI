...
sisfo_bp = Blueprint('sisfo_bp', __name__)
...
# S1 Sistem Informasi Prodi ID
ID_PRODI_SISTEM_INFORMASI = 3 # Corrected ID based on common mapping, previously 4 in provided code
...
@sisfo_bp.route('/dekannilaisisfo')
def dekan_nilai_sisfo():
    return render_template('dekan_nilai_S1sisfo.html')

# S1 SISTEM INFORMASI STUDENT DETAILS (FOR DEKAN_NILAI_S1SISFO.HTML)
@sisfo_bp.route('/api/mahasiswa-sisfo', methods=['GET'])
def api_mahasiswa_sisfo():
    ...
    for angkatan in angkatan_list:
        try:
            ...
            for mhs in data:
                if (
                    int(mhs.get("id_program_studi", -1)) == ID_PRODI_SISTEM_INFORMASI and
            ...
            logging.error(f"Error fetching data for angkatan {angkatan}: {e}")
        except ValueError as e:
            logging.error(f"Error parsing JSON for angkatan {angkatan}: {e}")
        except Exception as e:
            logging.error(f"An unexpected error occurred for angkatan {angkatan}: {e}")

    return jsonify(result_data)

# STUDENT DETAILS WITH GRAPH
@sisfo_bp.route('/detailnilaisisfo/<nim>')
def detail_mahasiswa(nim):
        ...
        if bio_data:
            ...
            for semester_num in range(1, semester_now + 1):
                ...
                try:
                    ...
                    logging.error(f"Error fetching histori for NIM {nim}, Periode {id_periode}: {e}")
                    histori.append({
                        "semester": f"Semester {semester_num}",
                        "ips": 0.0,
                        "ipk": 0.0
                    ...
                    logging.error(f"Error parsing histori JSON for NIM {nim}, Periode {id_periode}: {e}")
                    histori.append({
                        "semester": f"Semester {semester_num}",
                        "ips": 0.0,
                        "ipk": 0.0
    ...
    return render_template("detail_nilai_S1sisfo.html", mahasiswa=mahasiswa_detail)